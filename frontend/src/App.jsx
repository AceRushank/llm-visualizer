...
function AnalyticsPanel({ hoveredToken }) {
  const [barWidths, setBarWidths] = useState([])
  const prevTokenRef = useRef(null)

  // Trigger bar animation whenever the hovered token changes
  useEffect(() => {
    if (hoveredToken && hoveredToken !== prevTokenRef.current) {
      prevTokenRef.current = hoveredToken
      // Start at 0, then animate to target after a tick
      setBarWidths([])
  ...
  if (!hoveredToken) {
    return (
      <div className="analytics-idle">
        <p className="analytics-idle-text">
          Hover a token to inspect the model's prediction distribution at that position.
        </p>
        <p className="analytics-idle-sub">
          TinyLlama computes a Softmax distribution across 32,000 tokens at each position.
          The alternatives reveal what the model considered most likely before committing.
        </p>
      ...
      <div className="panels-grid">
        ...
        <div className="panel" aria-label="Token analytics">
          ...
          <div className="panel-body" style={{ position: 'relative' }}>
            ...
            {!loading && (
              hasData
                ? <AnalyticsPanel hoveredToken={hoveredToken} />
